...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, required=True, help="Path to ImageNet root directory"
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    parser.add_argument(
        "--img_size", type=int, default=256, help="Resolution to center-crop and resize"
    )
    ...
    data_loader = DataLoader(
        dataset_train, batch_size=256, num_workers=32, shuffle=False, pin_memory=False
    )
    ...
    for batch_images, batch_labels in tqdm(data_loader):
        for i in range(batch_images.size(0)):
            ...
            out_path = os.path.join(
                args.output_path, f"transformed_{global_idx:08d}.png"
            )
